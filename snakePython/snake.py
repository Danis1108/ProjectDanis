...
def print_new_game_message(screen):
    ...
    text1 = font.render("Нажмите ENTER что бы начать игру", True, TEXT_COLOR)
    text2 = font.render("Нажмите ESCAPE что бы выйти", True, TEXT_COLOR)
...
def print_game_paused_message(screen):
    font = pygame.font.SysFont("Courier New", FONT_SIZE, bold=True)
    text1 = font.render("Нажмите SPACE что бы продолжить", True, TEXT_COLOR)
    text2 = font.render("Нажмите ESCAPE что бы начать новую игру", True, TEXT_COLOR)
    text_rect1 = text1.get_rect()
    text_rect2 = text2.get_rect()
    text_rect1.center = (WIDTH // 2, HEIGHT // 2 - FONT_SIZE // 2)
    text_rect2.center = (WIDTH // 2, HEIGHT // 2 + FONT_SIZE // 2)
...
def draw_apples(screen, apples):
    for apple in apples:
        x = apple[0] * BLOCK_SIZE + WALL_SIZE * BLOCK_SIZE
        y = apple[0] * BLOCK_SIZE + WALL_SIZE * BLOCK_SIZE
